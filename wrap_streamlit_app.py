...
def scatter_plot(df, x_col, y_col, title, y_label=None, color_col=None, use_line=False):
    ...
    hover_cols = [
        c for c in [
            "test", "sample", "sample_type", "sample_date", "analysis_date", "aim_ph",
...
def heatmap(df, value_col, title, color_label):
    ...
    pivot = df.pivot_table(values=value_col, index="aim_ph", columns="alum_dose", aggfunc="mean")
...
def parameter_explorer(df):
    ...
    with c3:
        color_options = ["None"] + [c for c in ["aim_ph", "test", "sample", "sample_type", "sample_date", "analysis_date"] if c in df.columns]
        selected_color = st.selectbox("Color points by", color_options, index=1 if "aim_ph" in color_options else 0)
...
def generate_html_report(df, source_name="Traditional Jar Tests"):
    if df is None or df.empty:
        return "<html><body><h1>No data available</h1></body></html>"

    numeric = df.select_dtypes(include="number")
    summary_html = numeric.describe().to_html() if not numeric.empty else "<p>No numeric data available.</p>"
...
# -------------------- EXTRA PARAMETERS --------------------
    ...
    with c3:
        color_options = ["None"] + nums
        color_col = st.selectbox("Color By", color_options, index=color_options.index("aim_ph") if "aim_ph" in color_options else 0)
...
# -------------------- REPORTS --------------------
    ...
    html_report = generate_html_report(filtered_df, "Traditional Jar Tests")
    ...
    st.info("Open the HTML report in a browser and use Print > Save as PDF to export it as a PDF.")
