...
# ============================================================
# WRAP v22 - Water Research Analytics Platform
...
def robot_optimal_dose_module(
    ...
    The model uses the detected coagulant dose/volume field and compares the
    Initial measurement with the Final / Settling measurement for each robot
    sample. Removal curves are then aggregated by reported coagulant dose.
    """
    if filtered_long is None or filtered_long.empty:
    ...
    valid_first = curve.dropna(subset=["first_derivative"]).copy()
    significant_dose = None
    significant_gain = None
    ...
    threshold_candidates = valid_first[
        valid_first["first_derivative"] <= float(threshold)
    ].copy()
    if not threshold_candidates.empty:
        ...
        threshold_dose = float(
            curve.loc[curve["mean_removal"].idxmax(), dose_col]
        )

    method_rows = []
    if significant_dose is not None:
        method_rows.append({
            "Method": "Maximum First Derivative",
            "Recommended dose": significant_dose,
            "Interpretation": "Largest increase in removal between consecutive dose levels",
        })
    if diminishing_dose is not None:
        method_rows.append({
            "Method": "Diminishing Returns / Second Derivative",
            "Recommended dose": diminishing_dose,
            "Interpretation": "Strongest reduction in additional removal benefit",
        })
    method_rows.append({
        "Method": f"Marginal Gain Threshold ≤ {threshold:.2f}",
        "Recommended dose": threshold_dose,
        "Interpretation": "First dose at or below the selected marginal-gain threshold",
    })
    ...
    doses_for_consensus = [
        dose for dose in [significant_dose, diminishing_dose, threshold_dose]
        if dose is not None
    ]
    consensus_text = None
    if len(doses_for_consensus) >= 2:
        counts = {}
        for dose in doses_for_consensus:
            counts[dose] = counts.get(dose, 0) + 1
        consensus_dose, agreement_count = max(counts.items(), key=lambda item: item[1])
        if agreement_count >= 2:
            consensus_text = (
                f"{agreement_count} methods agree at {consensus_dose:g} "
                f"reported dose unit(s)."
            )
            st.success(consensus_text)
        elif len(doses_for_consensus) == 3:
            st.warning(
                "The three methods do not agree on one dose. Review the removal curve "
                "and the exact robot samples used before selecting an operating dose."
            )
    ...
    with metric_cols[0]:
        st.metric(
            "First Derivative",
            f"{significant_dose:g}" if significant_dose is not None else "N/A",
        )
    with metric_cols[1]:
        st.metric(
            "Diminishing Returns",
            f"{diminishing_dose:g}" if diminishing_dose is not None else "N/A",
        )
    with metric_cols[2]:
        st.metric("Threshold Recommendation", f"{threshold_dose:g}")
    ...
    if significant_dose is not None:
        fig.add_vline(
            ...
            line_dash="dash",
            line_color="black",
            annotation_text=f"First derivative: {significant_dose:g}",
            annotation_position="top left",
        )
    if diminishing_dose is not None:
        fig.add_vline(
            ...
            line_dash="dash",
            line_color="red",
            annotation_text=f"Diminishing returns: {diminishing_dose:g}",
    ...
    fig.add_vline(
        ...
        line_dash="dot",
        line_color="green",
        annotation_text=f"Threshold: {threshold_dose:g}",
