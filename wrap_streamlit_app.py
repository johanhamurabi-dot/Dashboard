...
def robot_parameter_sample_summary(
    ...
    """Build one initial/final summary row per robot sample for a parameter."""
    aggregated = aggregate_robot_timepoints(
        long_df, parameter=parameter, test_ids=test_ids
    )
    if aggregated.empty:
        return pd.DataFrame()
    ...
    for col in source.columns:
        ...
        if any(token in normalized for token in [
            "template", "date started", "date completed", "dose",
            "coagulant", "alum", "target ph", "aim ph"
        ]):
            if col not in {
    ...
    rows = []
    for test_id, points in aggregated.groupby("robot_test_id", dropna=False):
        initial = _robot_choose_point(points, "Initial")
        final = _robot_choose_point(points, "Final")
        if initial is None:
            initial = _robot_choose_point(points, "First available")
        if final is None:
            final = _robot_choose_point(points, "Last available")
        ...
        row = {
            "robot_test_id": test_id,
            "initial_point": initial["time_label"],
            "initial_value": initial["value"],
            "final_point": final["time_label"],
            "final_value": final["value"],
            "absolute_change": absolute_change,
            "percent_change": percent_change,
        }
        sample_meta = source[source["robot_test_id"] == test_id]
        if not sample_meta.empty:
            for col in metadata_cols:
                values = sample_meta[col].dropna()
                if not values.empty:
...
def robot_optimal_dose_module(
    ...
    """
    Robot-only Optimal Dose Finder for COD and Turbidity.

    This mirrors the decision-support options available in the Traditional
    Optimal Dose Finder while respecting the MANTECH robot data structure.
    The robot model compares Initial with Final / Settling measurements for
    each sample, calculates removal, groups results by the detected coagulant
    dose/volume field, and evaluates three dose-selection methods.
    """
    if filtered_long is None or filtered_long.empty:
    ...
    if not dose_col:
        st.warning(
            "WRAP could not detect a coagulant dose or coagulant volume field in "
            "the selected robot data. The Robot Optimal Dose Finder requires a "
            "reported dose field."
        )
        return
    ...
    available = [
        (key, label) for key, label in preferred_parameters if key in parameter_keys
    ]

    if not available:
        st.warning(
            "No COD or Turbidity measurements were detected in the selected robot data."
        )
        return
    ...
    if summary.empty or dose_col not in summary.columns:
        st.warning(
            "The selected parameter could not be paired with the detected coagulant "
            "dose field."
        )
        return
    ...
    work["initial_value"] = pd.to_numeric(work["initial_value"], errors="coerce")
    work["final_value"] = pd.to_numeric(work["final_value"], errors="coerce")

    total_rows = len(work)
    work = work.dropna(subset=[dose_col, "initial_value", "final_value"]).copy()
    work = work[work["initial_value"] != 0].copy()

    if work.empty:
        st.warning(
            "No valid robot samples contained a numeric dose plus valid Initial and "
            "Settling / Final measurements."
        )
        return

    work["removal_percent"] = (
        (work["initial_value"] - work["final_value"])
        / work["initial_value"]
    ) * 100
    ...
    if negative_final_count:
        st.warning(
            f"{negative_final_count} selected {selected_label} final measurement(s) are "
            "negative in the source data. WRAP keeps the original measurements and "
            "does not silently correct them; review these values before interpreting "
            "the optimal-dose recommendation."
        )

    curve = (
        work.groupby(dose_col, as_index=False)
        .agg(
    ...
    curve["second_derivative"] = (
        curve["first_derivative"].diff() / curve["dose_step"]
    )

    unique_doses = int(curve[dose_col].nunique())
    excluded_rows = total_rows - len(work)
    ...
    def _method_result(selected_threshold: float):
        ...
        significant_dose = float(significant_row[dose_col])
        significant_gain = float(significant_row["first_derivative"])

        valid_second = curve.dropna(subset=["second_derivative"]).copy()
        if valid_second.empty:
            diminishing_dose = significant_dose
            diminishing_second = None
        else:
            diminishing_row = valid_second.loc[valid_second["second_derivative"].idxmin()]
            diminishing_dose = float(diminishing_row[dose_col])
            diminishing_second = (
                float(diminishing_row["second_derivative"])
                if pd.notna(diminishing_row["second_derivative"])
                else None
            )
        ...
        if not threshold_candidates.empty:
            threshold_row = threshold_candidates.sort_values(dose_col).iloc[0]
        else:
        ...
        return {
            ...
            "threshold_dose": float(threshold_row[dose_col]),
            "threshold_gain": (
                float(threshold_row["first_derivative"])
                if pd.notna(threshold_row.get("first_derivative"))
                else None
            ),
    ...
    def _robot_curve_quality():
        repeated_doses = int((curve["n"] >= 2).sum()) if "n" in curve.columns else 0
        if unique_doses >= 5 and repeated_doses >= 2:
            return "Strong"
        if unique_doses >= 4:
            return "Acceptable"
        if unique_doses >= 3:
    ...
    def _robot_consensus(result):
        if not result:
            return {
                "consensus_dose": None,
                "agreement_count": 0,
                "agreement_text": "No consensus available.",
            }
        doses = [
            result.get("significant_dose"),
            result.get("diminishing_dose"),
            result.get("threshold_dose"),
        ]
        doses = [float(value) for value in doses if value is not None]
        if not doses:
            return {
                "consensus_dose": None,
                "agreement_count": 0,
        ...
        if agreement_count >= 2:
            return {
                ...
                "agreement_text": (
                    f"{agreement_count} of 3 methods agree at {consensus_dose:g} "
                    "reported dose unit(s)."
                ),
            }
        return {
            "consensus_dose": None,
            "agreement_count": agreement_count,
            "agreement_text": (
                "The three methods do not agree on one dose. Review the curve, "
                "dose-level variability, and threshold sensitivity."
            ),
        }
    ...
    with st.expander("🔎 Data Used for the Robot Optimal-Dose Calculation", expanded=True):
        ...
        with a1:
            st.metric("Robot rows available", total_rows)
        with a2:
            st.metric("Rows used", len(work))
        with a3:
            st.metric("Rows excluded", excluded_rows)
        with a4:
            st.metric("Distinct dose values", unique_doses)

        st.caption(
            f"Dose field detected from the MANTECH export: {dose_col}. "
            f"The {selected_label} model uses Initial and Settling / Final measurements "
            "from the currently filtered robot samples."
        )
        st.dataframe(
            work[[
                col for col in [
                    "robot_test_id", dose_col, "initial_point", "initial_value",
    ...
    # A single dose can be summarized but cannot define a dose-response slope.
    if unique_doses < 2:
        only_doses = ", ".join(
            map(str, curve[dose_col].dropna().unique().tolist())
        )
        st.warning(
            "An optimal dose cannot be calculated from a single coagulant dose. "
            f"The current filtered robot data contain {unique_doses} distinct dose "
            f"value(s): {only_doses}. Include robot tests with at least two different "
            "dose values to calculate a dose-response slope, and preferably three or "
            "more for the full three-method comparison."
        )
        ...
        fig_single = px.scatter(
            ...
            y="removal_percent",
            color="robot_test_id",
            title=f"{selected_label} Removal by Reported Coagulant Dose",
            labels={"removal_percent": f"{selected_label} removal (%)"},
            hover_data=["initial_value", "final_value"],
        )
        fig_single.update_traces(marker=dict(size=11))
        render_plot(fig_single, "robot_optimal_single_dose")
        st.dataframe(curve, use_container_width=True, hide_index=True)
        return

    result = _method_result(float(threshold))
    if result is None:
        st.warning("Not enough dose-response information is available to calculate an optimal dose.")
        return

    consensus = _robot_consensus(result)
    quality = _robot_curve_quality()
    ...
    with c1:
        metric_card(
            f"{result['significant_dose']:g}",
            "First Derivative",
            "metric-card",
        )
    with c2:
        metric_card(
            f"{result['diminishing_dose']:g}",
            "Diminishing Returns",
            "metric-card-red",
        )
    with c3:
        metric_card(
            f"{result['threshold_dose']:g}",
    ...
    fig = px.line(
        ...
        labels={"mean_removal": f"Mean {selected_label} removal (%)"},
        hover_data=[
            "n", "mean_initial", "mean_final", "first_derivative", "second_derivative"
        ],
    )
    fig.update_traces(line=dict(width=3), marker=dict(size=10))
    fig.add_vline(
        x=result["significant_dose"],
        line_dash="dash",
        line_color="black",
        annotation_text=f"First derivative: {result['significant_dose']:g}",
        annotation_position="top left",
    )
    fig.add_vline(
        x=result["diminishing_dose"],
        line_dash="dash",
        line_color="red",
        annotation_text=f"Diminishing returns: {result['diminishing_dose']:g}",
        annotation_position="top right",
    )
    fig.add_vline(
        x=result["threshold_dose"],
        line_dash="dot",
        line_color="green",
        annotation_text=f"Threshold: {result['threshold_dose']:g}",
    ...
    variability_table = curve[[
        dose_col, "mean_initial", "mean_final", "sd_final", "mean_removal",
        "sd_removal", "n", "first_derivative", "second_derivative"
    ]].copy()
    variability_table = variability_table.rename(columns={
        dose_col: "Reported Dose",
    ...
    st.caption(
        "Robot Samples (n) is the number of distinct robot samples available at each "
        "reported dose. It is not automatically interpreted as a laboratory replicate count."
    )
    ...
    with c_final:
        fig_final = px.line(
            ...
            labels={"mean_final": f"Mean final {selected_label}"},
            hover_data=["n"],
        )
        fig_final.update_traces(line=dict(width=3), marker=dict(size=10))
    ...
    with c_gain:
        fig_gain = px.line(
            curve.dropna(subset=["first_derivative"]),
            x=dose_col,
            y="first_derivative",
            markers=True,
            title=f"Marginal {selected_label} Removal Gain",
            labels={"first_derivative": "Removal gain per reported dose unit"},
        )
        fig_gain.update_traces(line=dict(width=3), marker=dict(size=10))
    ...
    with s1:
        threshold_min = st.number_input(
            "Sensitivity range minimum",
            min_value=0.0,
            max_value=100.0,
            value=0.5,
            step=0.25,
            key="robot_sensitivity_min",
        )
    with s2:
        threshold_max = st.number_input(
            "Sensitivity range maximum",
            min_value=0.0,
            max_value=100.0,
            value=3.0,
            step=0.25,
            key="robot_sensitivity_max",
        )
    with s3:
    ...
    if threshold_max < threshold_min:
        ...
        if sensitivity_df.empty:
            ...
            sensitivity_fig = px.line(
                ...
                labels={
                    "threshold": "Marginal Gain Threshold",
                    "threshold_dose": "Recommended Dose",
                },
            )
            ...
            if unique_recommendations == 1:
                ...
                st.success(
                    f"The threshold recommendation is robust across the selected range: "
                    f"{stable_dose:g} reported dose unit(s)."
                )
            elif unique_recommendations <= 2:
                st.info(
                    "The recommendation is moderately robust: only two doses are selected "
                    "across the tested threshold range."
                )
            else:
                st.warning(
                    "The recommended robot dose is sensitive to the threshold choice. "
                    "Review the curve before standardizing an operating dose."
                )

    st.subheader("Automatic Robot Test Summary")
    threshold_gain = result.get("threshold_gain")
    threshold_gain_text = (
        "N/A" if threshold_gain is None or pd.isna(threshold_gain)
        else f"{threshold_gain:.2f}"
    )
    dose_counts = curve[[dose_col, "n"]].copy()
    repeated = dose_counts[dose_counts["n"] >= 2]
    if repeated.empty:
        coverage_text = "No dose level contains more than one robot sample."
    else:
        coverage_text = ", ".join(
            f"{row[dose_col]:g} (n={int(row['n'])})"
            for _, row in repeated.iterrows()
        )
        coverage_text = f"Dose levels with multiple robot samples: {coverage_text}."

    summary_text = f"""
**Robot Optimal-Dose Summary — {selected_label}**

- **Dose field:** {dose_col}
- **Robot samples used:** {len(work)}
- **Distinct dose levels:** {unique_doses}
- **First-derivative dose:** {result['significant_dose']:g}
- **Diminishing-returns dose:** {result['diminishing_dose']:g}
- **Threshold dose ({threshold:.2f}):** {result['threshold_dose']:g}
- **Marginal gain at threshold dose:** {threshold_gain_text}
- **Method agreement:** {consensus['agreement_text']}
- **Curve quality:** {quality}
- **Dose-level coverage:** {coverage_text}
