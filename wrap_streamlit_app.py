...
# ============================================================
# WRAP v13 - Water Research Analytics Platform
# Dalhousie Water Research
# Traditional Jar Tests + Expanded Parameters + Dose Response + Removal + Marginal Gain
...
def parameter_specs():
    ...
    return [
        ...
        ("tthms_removal_percent", "TTHMs Change", "TTHMs change/removal (%)", False),
        ("thaas", "THAAs", "THAAs", False),
        ("thaas_removal_percent", "THAAs Change", "THAAs change/removal (%)", False),
...
def optimal_dose_module(df):
    """
    Reviewer-requested optimal alum dose module.
    Starts with marginal gain threshold and also displays derivative-based points.
    """
    if df is None or df.empty:
    ...
    if not available:
        st.warning("No removal curves available. Make sure raw water rows or alum_dose = 0 rows are included.")
        return
    ...
    with c1:
        selected_label = st.selectbox("Optimization parameter", list(label_to_param.keys()), index=0)
    with c2:
        threshold = st.number_input(
            "Marginal gain threshold (%/mg/L)",
            min_value=0.0,
            max_value=10.0,
            value=1.0,
            step=0.25
    ...
    if result is None or curve.empty:
        st.warning("Not enough data to calculate optimal dose.")
        return

    c1, c2, c3 = st.columns(3)
    with c1:
        metric_card(f"{result['significant_dose']:.2f} mg/L", "Black: Most Significant Improvement", "metric-card")
    with c2:
        metric_card(f"{result['diminishing_dose']:.2f} mg/L", "Red: Diminishing Returns", "metric-card-red")
    with c3:
    ...
    fig = px.line(
        ...
        labels={
            ...
            "mean_removal": f"Mean {selected_label} Removal (%)",
            "sd_removal": "SD"
        },
    ...
    fig.add_vline(
        ...
        line_dash="dash",
        line_color="black",
        annotation_text=f"1st derivative: {result['significant_dose']:.2f}",
    ...
    fig.add_vline(
        ...
        line_dash="dash",
        line_color="red",
        annotation_text=f"2nd derivative: {result['diminishing_dose']:.2f}",
    ...
    fig.add_vline(
        ...
        line_dash="dot",
        line_color="green",
        annotation_text=f"≤ {threshold:.2f}%/mg: {result['threshold_dose']:.2f}",
        annotation_position="bottom right"
    )

    render_plot(fig, "optimal_dose_curve")

    st.subheader("Derivative Table")
    ...
    st.markdown(
        ...
        **Mathematical interpretation**
        ...
        - **First derivative / marginal gain:** ΔRemoval% / ΔAlum dose. This identifies where the curve improves the fastest.
        - **Second derivative:** ΔMarginal gain / ΔAlum dose. This helps identify where returns begin to diminish.
        - **Marginal gain threshold:** the selected dose is the first dose where marginal gain is no more than **{threshold:.2f}% removal per mg/L alum**.
        """
    )
