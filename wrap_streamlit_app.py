...
# ============================================================
# WRAP v21 - Water Research Analytics Platform
...
def load_robot_file(uploaded_file):
    ...
    try:
        ...
        run_metadata_patterns = [
            r"^run template$", r"^run id$", r"^date started$", r"^date completed$"
        ]
        run_metadata_cols = [
...
def robot_data_module():
    ...
    st.caption(
        ...
        "Initial measurements, RPM 1/2/3 stages, settling results, replicates, and run metadata."
    )
    ...
    if long_df is None or long_df.empty:
        st.error(
            ...
            "Expected the MANTECH format with fields such as Sample ID, Initial pH, "
            "RPM 1 pH 0min, Settling Turbidity, Conductivity, or COD."
        )
    ...
    c1, c2, c3, c4 = st.columns(4)
    with c1:
        metric_card(len(raw_df), "🤖 Robot Samples", "metric-card")
    with c2:
        run_count = raw_df["robot_run_id"].nunique() if "robot_run_id" in raw_df.columns else "N/A"
        metric_card(run_count, "🧾 Robot Runs", "metric-card-blue")
    with c3:
        metric_card(long_df["parameter"].nunique(), "🧪 Parameters Found", "metric-card-green")
    with c4:
        metric_card(long_df["process_label"].nunique(), "⚙️ Process Points", "metric-card-red")

    st.success(
        "MANTECH Jar Testing format detected. Blank run-level cells were reconstructed, "
        "and each Sample ID is treated as an independent robot sample."
    )

    st.subheader("Robot Sample Filters")
    ...
    if not selected_tests:
        st.warning("Select at least one robot sample.")
        return
    ...
    with overview_tab:
        ...
        structure = (
            ...
            .agg(
                ...
                samples=("robot_test_id", "nunique"),
                process_points=("process_label", "nunique"),
                first_process_order=("analysis_order", "min"),
                last_process_order=("analysis_order", "max"),
            )
            .reset_index()
        )
        ...
        st.markdown("**Process points detected from the uploaded headers**")
        st.dataframe(process_map, use_container_width=True, hide_index=True)

        duplicates = (
            filtered_long.groupby(
                ["robot_test_id", "parameter_label", "process_label"], dropna=False
            )
            .size()
        ...
        if duplicates.empty:
            st.info("No repeated measurements were detected at the same sample and process point.")
        else:
        ...
        with st.expander("Original robot export"):
            st.dataframe(
                raw_df[raw_df["robot_test_id"].isin(selected_tests)].head(150),
                use_container_width=True,
            )
    ...
    with timeline_tab:
        ...
        st.markdown("**Aggregated values by detected process point**")
        st.dataframe(table, use_container_width=True)
        st.caption(
            "The file labels every RPM measurement as 0 min inside its own stage. "
            "WRAP therefore plots the real process order and keeps that 0 min value as stage-local time."
        )
    ...
    with heatmap_tab:
        ...
        st.caption(
            "Blank cells mean that a robot sample did not contain that parameter at the selected process point."
        )
    ...
    with compare_tab:
        ...
        if comparison.empty:
            st.warning("No samples contained both requested process points.")
        else:
    ...
    with report_tab:
        ...
        st.info(
            "This report belongs only to the robot workflow. The Traditional Jar Tests section and its calculations remain unchanged."
        )
